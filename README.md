...
Generated combat feedback uses a second four-Pack chain: `Effect Selector → Effect Atlas → Godot VFX Player → Effect Receipt`. Four transparent ImageGen source sheets—warrior slash, archer impact, healer restoration, and enemy claw—are hash-locked and normalized into one 1728×768 atlas containing nine forward-playback frames per effect. Both Godot profiles load the same atlas while preserving profile-specific receipts, and the existing Visual Maker/Verifier slots record the additional bindings without increasing the 13-agent ceiling. Local approved-Godot qualification passed both profiles with atlas-load, receipt, and frame-change checks; see [`generated-vfx-local-qualification-20260822.json`](docs/evidence/capability-pack-composition/generated-vfx-local-qualification-20260822.json). This remains local evidence until a separately authorized Cloud requalification.
