"""Receipt-gated workflow for the separate Godot visual-prototype ToolPack."""
...
class GodotVisualTeam(Protocol):
    call_count: int
...
def _blocked(criteria: list[str], issues: list[str]) -> AgentVerification:
    detail = "; ".join(issues) or "Godot visual-prototype verification failed"
    return AgentVerification(
        findings=[CriterionFinding(criterion=item, passed=False, evidence=detail) for item in criteria],
        verdict="REPAIR",
        repair_instructions=issues or ["Repair the bounded Godot visual prototype."],
    )


async def execute_godot_visual_run(
    run_id: str,
    *,
    store: RunStore,
    team: GodotVisualTeam,
    ...
    try:
        ...
        if toolpack.manifest.toolpack_id != "godot.visual-prototype":
            raise PermissionError("Godot visual workflow received the wrong ToolPack")
        if tuple(sorted(brief.authorized_output_files)) != toolpack.manifest.output.authorized_paths:
            raise PermissionError("approved output files do not match the Godot visual ToolPack manifest")
        if record.work_mode != "new_product_build" or record.source_snapshot is not None:
            raise PermissionError("Godot Visual Prototype ToolPack authorizes new products only")

        record = record.model_copy(update={
            "status": RunStatus.PLANNING,
            "message": "Godot Project Owner is binding a visual-prototype Quest chain.",
        })
        store.update(record)
        ...
        record = record.model_copy(update={
            ...
            "message": "Visual Director is issuing three distinct Godot visual concepts.",
        ...
        visual_plan = await team.plan_visuals({
            ...
            "render_surface": "Godot 4.7.1 at 1280x720",
        })
        store.put_json(run_id, "visuals/concept_plan.json", visual_plan.model_dump(mode="json"))

        eligible: dict[str, tuple[CompiledGodotVisualPrototype, object]] = {}
        ...
        for concept in visual_plan.candidates:
            record = record.model_copy(update={
                "status": RunStatus.VISUALIZING,
                "message": f"Nano Banana is generating Godot candidate {concept.candidate_id}: {concept.name}.",
                "model_calls": team.call_count,
            })
            store.update(record)
            ...
            if not gate.approved:
                evidence_payload[concept.candidate_id] = {
                    "passed": False,
                    "issues": gate.issues,
                    "asset_gate": gate.model_dump(mode="json"),
                }
                continue
            artifact = compile_godot_visual_prototype(decision.topology, concept, asset)
            with tempfile.TemporaryDirectory(prefix=f"khalinos-godot-visual-{run_id}-{concept.candidate_id}-") as temporary:
                ...
                capture_files = sorted(evidence_dir.glob("godot-render????????.png"))
                if capture_files:
                    capture = capture_files[-1]
                    screenshot_paths[concept.candidate_id] = store.put_file(
                        run_id, f"visuals/{concept.candidate_id}/evidence/{capture.name}", capture, "image/png"
                    )
                    screenshots.append((concept.candidate_id, capture.read_bytes()))
            evidence_payload[concept.candidate_id] = {
        ...
        if len(eligible) < 2:
            raise RuntimeError("fewer than two Godot visual candidates passed real rendering")
        record = record.model_copy(update={
            "status": RunStatus.VISUAL_SELECTING,
            "message": "Independent Visual Verifier is comparing real Godot renders.",
            "model_calls": team.call_count,
        })
        store.update(record)
        eligible_screenshots = [item for item in screenshots if item[0] in eligible]
        selection = await team.select_visual(
            {
                "approved_brief": brief.model_dump(mode="json"),
                "visual_concept_plan": visual_plan.model_dump(mode="json"),
                "render_surface": "real Godot display-backed PNG",
        ...
        if assessed != list(eligible) or selection.selected_candidate_id not in eligible:
            raise ValueError("Visual Verifier must assess and select exactly the eligible candidates")
        ...
        for quest in plan.quests:
            record = record.model_copy(update={
                ...
                "message": f"Independent Godot Verifier is checking {quest.quest_id} against rendered evidence.",
                "model_calls": team.call_count,
            })
            store.update(record)
            ...
            if not passed:
                record = record.model_copy(update={
                    ...
                    "message": f"{quest.quest_id} stopped; Godot visual evidence did not pass.",
                    "model_calls": team.call_count,
                })
                store.update(record)
                return record
        ...
        with tempfile.TemporaryDirectory(prefix=f"khalinos-godot-visual-final-{run_id}-") as temporary:
        ...
        record = record.model_copy(update={
            ...
            "completed_receipt_ids": receipt_ids,
            "message": "Godot visual prototype passed asset gating, real rendering, and independent verification.",
            "model_calls": team.call_count,
        })
        store.update(record)
